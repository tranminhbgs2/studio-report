...
<head>
    ...
    <script type="module">
        ...
        // Fetch your API_KEY
        const API_KEY = "AIzaSyB_irOZnLGjnvi61yZOzP5oKkycc02gODQ";
        ...
        window.run = async function() {
            ...
            if (msg) {
                ...
                msgElement.classList.add("message", "user"); // Đánh dấu tin nhắn là do người dùng gửi
                ...
                const replyingMessageElement = document.createElement("div");
                replyingMessageElement.classList.add("message", "replying");
                chatBox.appendChild(replyingMessageElement);
                let typingText = "Đang trả lời...";
                displayTextByCharacter(typingText, replyingMessageElement, async () => {
                    // Xóa thông báo "Đang trả lời..." sau khi hiển thị xong
                    try {
                        ...
                        const response = await result
                            .response; // Giả định result.response là dữ liệu bạn cần
                        const responseText = await response.text();
                        console.log(response.text());
                        receiveBotReply(responseText, replyingMessageElement); // Nhận và hiển thị câu trả lời từ bot

                    } catch (error) {
                        console.error("Đã xảy ra lỗi:", error);
                        // Xử lý phản hồi khi gặp lỗi, ví dụ: hiển thị thông báo lỗi cho người dùng
                    } finally {
                        // Kích hoạt lại nút sau khi hàm run hoàn thành hoặc xảy ra lỗi
        ...
        async function receiveBotReply(responseText, replyingMessageElement) {
            replyingMessageElement.remove();
            const chatBox = document.getElementById("chatBox");
            // Tạo phần tử để hiển thị phản hồi từ bot
            const botElement = document.createElement("div");
            botElement.classList.add("message", "bot");
            chatBox.appendChild(botElement);

            // Hiển thị câu trả lời từ bot từng chữ một
            displayTextByCharacter(responseText, botElement, () => {
                chatBox.scrollTop = chatBox.scrollHeight; // Cuộn đến tin nhắn mới nhất
            });
        }

        function displayTextByCharacter(text, element, callback) {
            let index = 0;
            const intervalId = setInterval(() => {
                if (index < text.length) {
                    ...
                    callback(); // Gọi callback sau khi hiển thị xong văn bản
                }
            }, 50); // Điều chỉnh tốc độ hiển thị từng chữ
        }
        // run();
    </script>
</head>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mt-8 ">
                <div class="input-group">
                    ...
                    <button id="btn-send" onclick="run()">Gửi</button>
                </div>
            </div>
            <div class="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
